...
public class SampleAsyncCallBack implements MqttCallback {
	...
	private static boolean receivedTemp = false;
	private static boolean receivedPres = false;
	private static boolean receivedDoor = false;
	private static boolean receivedWar = false;
	...
	public void subscribe(String topicName, int qos) throws Throwable {
		...
		while (state != FINISH) {
			...
			case CONNECTED:
				...
				break;
			case SUBSCRIBED:
				// Block until unsubscribed is pressed allowing messages to arrive
				log("Subscribed, waiting for unsubscribed button");
				while (MainActivity.subscribed){
					//Wait till unsubscribed is pressed
				}
				unsubscribe();
				break;
			case DISCONNECT:
	...
	public void messageArrived(String topic, MqttMessage message) throws MqttException {
		...
		if (topic.equals("sensors/door")){
			...
			MainActivity.readDoor = new String (message.getPayload());
			receivedDoor = true;
			...
			MainActivity.readTemp = new String (message.getPayload());
			...
			MainActivity.readPress = new String (message.getPayload());
			...
			MainActivity.readWarning = new String (message.getPayload());

			receivedWar = true;

		}else if (topic.equals("sensors/altitude")){
			MainActivity.readAlt = new String (message.getPayload());

		}else{
			//Change anything
			MainActivity.readTemp = ("?");
			MainActivity.readWarning = ("?");
			MainActivity.readDoor = ("?");
			MainActivity.readPress = ("?");
		...
		public void doSubscribe(String topicName, int qos) {
			...
			IMqttActionListener subListener = new IMqttActionListener() {
				...
				public void onFailure(IMqttToken asyncActionToken, Throwable exception) {
					...
					System.out.println("Subscribe failed: "+exception);
			...
			try {
				//TODO If we get here we are subscribing
